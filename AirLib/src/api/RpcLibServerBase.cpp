    ...
    : api_provider_(api_provider)
{
    ...
    pimpl_->server.
        bind("simGetAllSegmentationObjectIDs", [&]() -> std::vector<RpcLibAdapatorsBase::AllSegmentationIDsResponse> {
        const auto& response = getWorldSimApi()->getAllSegmentationObjectIDs();
        return RpcLibAdapatorsBase::AllSegmentationIDsResponse::from(response);
    });
